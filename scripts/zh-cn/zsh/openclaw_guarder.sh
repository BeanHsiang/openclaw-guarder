#!/usr/bin/env zsh
# =============================================================================
# OpenClaw Guarder — macOS 安全加固与配置检查脚本
# =============================================================================
...
flush_pending_commands() {
  ...
  if $DRY_RUN; then
    ...
    for (( i = 1; i <= total; i++ )); do
      log_info "[${i}/${total}] 执行：${PENDING_DESCS[$i]}"
...
harden_gateway_port() {
  ...
  # ── 交互式端口选择 ────────────────────────────────────────────────────────────
  print ""
  print "  ${BOLD}请选择端口修改方式：${RESET}"
  print "  ${BOLD}[1]${RESET} 随机生成一个五位端口号（10000–65535）"
  print "  ${BOLD}[2]${RESET} 手动输入端口号"
  print "  ${BOLD}[3]${RESET} 保持现状，不修改端口"
  print ""

  local choice
  read "choice?  请输入选项 [1/2/3]（默认 1）: "
  choice="${choice:-1}"
  ...
  case "$choice" in
    1)
      ...
      ;;
    2)
      read "new_port?  请输入新端口号（1024–65535）: "
...
harden_apikey_to_env() {
  ...
  local -a hits=()
  # zsh 不支持 bash 的 mapfile，使用 while read 逐行填充数组
...
harden_credentials_perms() {
  ...
  local profile
  # zsh glob 无匹配时会报错，用 (N) 修饰符让其静默返回空
