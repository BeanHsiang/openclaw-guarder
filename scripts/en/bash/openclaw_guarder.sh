...
# =============================================================================
# OpenClaw Guarder — Windows/Linux security hardening and configuration checker (Bash)
# =============================================================================
...
# Command queue (used for dry-run preview and apply execution)
...
check_package_manager() {
  log_section "Check package manager"

  if command -v apt-get &>/dev/null; then
    log_ok "Detected apt-get (Debian / Ubuntu)"
  elif command -v dnf &>/dev/null; then
    log_ok "Detected dnf (Fedora / newer RHEL)"
  elif command -v yum &>/dev/null; then
    log_ok "Detected yum (RHEL / CentOS)"
  elif command -v pacman &>/dev/null; then
    log_ok "Detected pacman (Arch Linux)"
  elif command -v zypper &>/dev/null; then
    log_ok "Detected zypper (openSUSE)"
  elif command -v brew &>/dev/null; then
    log_ok "Detected Homebrew / Linuxbrew  →  $(brew --prefix)"
  else
    log_warn "No common package manager detected (apt/dnf/yum/pacman/zypper/brew); verify your Node.js and pnpm install path manually"
  fi
}
...
check_node() {
  ...
  if _check_cmd node --version; then
    ...
    log_error "Node.js is not installed. Install it with your system package manager, for example: apt install nodejs npm"
    CHECKS_PASSED=false
  fi
...
# Entry: run all prerequisite checks
...
# Check OpenClaw Gateway process/service
...
# Check core OpenClaw config file
...
# Check whether default port 18789 is exposed (0.0.0.0)
check_port_exposure() {
...
# Check config file for usage of default port 18789
...
# ─────────────────────────────────────────────────────────────────────────────
# Module 3: Hardening function set (individual functions for menu usage)
# ─────────────────────────────────────────────────────────────────────────────

# General: backup file — executed immediately in apply mode; previewed in dry-run
_backup_file() {
  local target="$1"
...
# General: add command to queue (both dry-run and apply just enqueue)
_apply_or_preview() {
  local description="$1"
  shift
  local cmd_str
...
# Show or execute all queued commands
flush_pending_commands() {
  ...
  if $DRY_RUN; then
    ...
    for (( i = 0; i < total; i++ )); do
      log_info "[$((i+1))/${total}] Executing: ${PENDING_DESCS[$i]}"
...
# ── Hardening 1: disable anonymous/public tokens ──────────────────────────────
harden_gateway_auth() {
  log_section "Ensure Gateway authentication is enabled"
  # Reference: https://docs.openclaw.ai/gateway/configuration-reference#gateway
  # OpenClaw controls auth via gateway.auth.mode: legal values: none | token | password | trusted-proxy
  # mode: "none" means no auth; anyone who can access the port may use the Gateway.
  local config_file="${OPENCLAW_CONFIG_FILE}"

  # Detect mode: "none" in JSON5 or JSON styles
...
# ── Gateway part 2: non-default port check and modification ─────────────────
harden_gateway_port() {
  ...
  printf "\n"
  printf "  %bChoose port modification method:%b\n" "${BOLD}" "${RESET}"
  printf "  %b[1]%b Generate a random five-digit port (10000–65535)\n" "${BOLD}" "${RESET}"
  printf "  %b[2]%b Enter a port manually\n" "${BOLD}" "${RESET}"
  printf "  %b[3]%b Keep current port\n" "${BOLD}" "${RESET}"
  printf "\n"
...
# ── Hardening 2: network binding restrictions ───────────────────────────────
harden_network_bindings() {
  log_section "Gateway binding hardening"
  # Goal: ensure gateway.mode == "local" and gateway.bind == "loopback"
  local config_file="${OPENCLAW_CONFIG_FILE}"
...
# ── Part 1.1: openclaw.json file and directory permissions ───────────────────
harden_config_file_perms() {
  log_section "openclaw.json permissions"
  # Goal: config file readable/writable by owner only (600), config dir accessible by owner only (700)
  local config_file="${OPENCLAW_CONFIG_FILE}"
...
# ── Part 1.2: .env env file permissions ──────────────────────────────────────
harden_env_file_perms() {
  log_section ".env environment file permissions"
  # Goal: .env contains API keys and must be owner-only (600)
...
# ── Part 1.3: migrate models.providers.*.apiKey into .env ───────────────────
harden_apikey_to_env() {
  log_section "Models Providers API Key detection & migration"
  # Goal: scan openclaw.json for models.providers.<name>.apiKey plaintext values and:
  #  1) append <PROVIDER_UPPER>_API_KEY=<value> to .env
  #  2) replace plaintext value in openclaw.json with ${<PROVIDER_UPPER>_API_KEY}
  local config_file="${OPENCLAW_CONFIG_FILE}"
  local env_file="${OPENCLAW_ENV_FILE}"
  local env_dir
...
# ── agents.defaults.compaction checks ───────────────────────────────────────
optimize_compaction() {
...
# ── Part 1.3: credentials and auth config permissions ───────────────────────
harden_credentials_perms() {
  log_section "Credentials & auth files permissions"
  # Goal: channel credentials and auth-profiles.json owner-only (600)
  local config_dir="${OPENCLAW_CONFIG_DIR}"
...
# ── Logging sensitive redact checks ─────────────────────────────────────────
...
# ── Gateway Control UI dangerous flags checks ──────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Module 4: Hardening summary entry (run all hardening steps)
# ─────────────────────────────────────────────────────────────────────────────
run_hardening() {
...
main() {
  ...
  printf "╚══════════════════════════════════════════╝%b\n" "${RESET}"
  printf "  Mode: %s\n" "$(if $DRY_RUN; then echo 'DRY-RUN (preview)'; elif $APPLY; then echo 'APPLY (apply hardening)'; else echo 'Check (read-only)'; fi)"
  printf "  Time: %s\n\n" "$(date '+%Y-%m-%d %H:%M:%S')"
